  ...
  const handleDelete = async (location: Location) => {
    ...
    if (confirm(`${t('locations.confirmDelete')}: ${location.name}`)) {
      ...
      try {
        ...
        alert(error instanceof Error ? error.message : 'Error al eliminar ubicación')
        setDeleting(null)
      }
    }
  }
      ...
      <div style={{ display: 'grid', gap: '16px' }}>
        ...
        {locations.length === 0 && (
          <div style={{
            ...
            <p>No hay ubicaciones creadas. Crea la primera ubicación para organizar tu inventario.</p>
          </div>
        )}
      </div>
    </main>
