...
export function BOMManualEntry({ existingSiteKitId, onImportComplete, onClose }: BOMManualEntryProps) {
  ...
  const handleSave = async () => {
    ...
    if (!existingSiteKitId && !header.siteKitId.trim()) {
      setError('Site Kit ID es requerido')
      return
    }
    if (validItems.length === 0) {
      setError('Agrega al menos un item con SKU')
      return
    }
    ...
    try {
      if (existingSiteKitId) {
        ...
        if (!res.ok) {
          let msg = 'Error al crear Site Kit'
    ...
    <div style={{
      ...
      <div className="card" style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
            ...
            <div>
              <h2 className="heading-lg" style={{ margin: 0 }}>
                {existingSiteKitId ? 'Agregar Items al Site Kit' : 'Entrada Manual de BOM'}
              </h2>
              <p style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', margin: 0 }}>
                Formato Tellworks — ingresa los datos del documento BOM
              </p>
            </div>
          </div>
        ...
        <div style={{ marginBottom: '1rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
            <h3 style={{ fontSize: '0.875rem', fontWeight: 600, color: 'var(--text-secondary)', margin: 0 }}>
              Items del BOM ({items.filter(i => i.siteKitSku.trim()).length} items)
            </h3>
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              ...
              >
                <Copy size={14} /> Pegar desde tabla
              </button>
          ...
          {showBulkPaste && (
            <div style={{
              ...
              <p style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginBottom: '0.5rem' }}>
                Pega filas desde Excel o texto. Formato por linea: <strong>SKU, Cantidad, Descripción, AssetTags...</strong>
              </p>
              <textarea
                value={bulkText}
          ...
          <div style={{ border: '1px solid var(--border-light)', borderRadius: '10px', overflow: 'hidden' }}>
            ...
            <div style={{
              ...
              <span>Asset Tags (separados por coma)</span>
              <span></span>
            </div>
        ...
        <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'flex-end' }}>
          ...
          >
            {saving ? <><Loader2 size={16} className="animate-spin" /> Guardando...</> : `Guardar BOM (${items.filter(i => i.siteKitSku.trim()).length} items)`}
          </button>
        </div>
      </div>
    </div>
